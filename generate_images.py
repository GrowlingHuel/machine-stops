...
def generate_image(
    ...
    first reference image. extra_refs adds additional reference images before
    the prompt (useful for style-consistency across a cluster).
    ...
    # Primary predecessor reference
    if reference_bytes:
        contents.append(
            types.Part.from_bytes(data=reference_bytes, mime_type="image/jpeg")
        )

    # Extra references (e.g. all other airship images for consistency)
    if extra_refs:
    ...
    # Prefix prompt with reference instruction if any refs provided
    if reference_bytes or extra_refs:
        ref_count = (1 if reference_bytes else 0) + len(extra_refs or [])
        contents.append(
            f"Here {'are' if ref_count > 1 else 'is'} {ref_count} reference "
            f"image{'s' if ref_count > 1 else ''} from the same cluster. "
            f"Generate a variant that matches their architectural style, "
            f"Art Nouveau decorative vocabulary, colour palette, and composition "
            f"language precisely, with only the specific changes described below:\n\n"
            + prompt
        )
    else:
...
def run(args):
    ...
    if getattr(args, 'single', None):
        ...
        # Find which cluster and position this image lives at
        cluster_name = None
        predecessor  = None
        for cname, files in CLUSTER_ORDER:
            if target in files:
                cluster_name = cname
                idx = files.index(target)
                predecessor = files[idx - 1] if idx > 0 else None
                break

        if cluster_name is None:
            print(f"✗ '{target}' not found in any cluster. Check the filename.")
            sys.exit(1)

        if target not in prompts:
            print(f"✗ No prompt found for '{target}' in IMAGE_PROMPTS.md")
            sys.exit(1)

        # Load predecessor as reference if it exists on disk
        ref_bytes = None
        if predecessor:
            pred_path = IMAGES_DIR / predecessor
            if pred_path.exists():
                ref_bytes = pred_path.read_bytes()
                print(f"  Using {predecessor} as reference image")
            else:
                print(f"  ⚠ Predecessor {predecessor} not found on disk — generating standalone")

        out_path = IMAGES_DIR / target
        ref_label = f" (referencing {predecessor})" if ref_bytes else " (standalone)"
        print(f"  Generating {target}{ref_label}...", end=" ", flush=True)

        if args.dry_run:
            print(f"\n  Prompt: {prompts[target][:200]}...")
            return
        ...
        # Load any extra --refs images
        extra_ref_bytes = []
        for ref_path in (getattr(args, 'refs', None) or []):
            p = Path(ref_path)
            if p.exists():
                extra_ref_bytes.append(p.read_bytes())
                print(f"  Extra reference: {ref_path}")
            else:
                print(f"  ⚠ Reference not found: {ref_path}")

        image_bytes = generate_image(
            client, prompts[target], model=args.model,
            reference_bytes=ref_bytes,
            extra_refs=extra_ref_bytes or None,
            debug=getattr(args, 'debug', False),
        )
        if image_bytes:
            save_image(image_bytes, out_path)
            print(f"✓  saved to images/{target}")
        else:
            print("✗  failed")
        return
...
def main():
    ...
    parser.add_argument(
        ...
        help="Regenerate one specific image, e.g. --single kuno_cell_shaft.jpg. "
             "Automatically loads the preceding cluster image as a reference."
    )
    parser.add_argument(
        "--refs", metavar="FILE", nargs="+",
        help="Extra reference images to pass alongside --single, e.g. "
             "--refs images/airship_blind.jpg images/airship_night.jpg"
    )
    parser.add_argument(
